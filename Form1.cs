...
namespace SoD2_Editor
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            // Безопасное переключение демо-режима
            chkIsDemo.MouseUp += (s, e) => {
                IntPtr addr = (IntPtr)addresses.Get("ULIsDemo");
                if (addr != IntPtr.Zero) WUInt8(addr, (byte)(1 - RUInt8(addr)));
            };
            chkIsDemo.KeyUp += (s, e) => {
                IntPtr addr = (IntPtr)addresses.Get("ULIsDemo");
                if (addr != IntPtr.Zero) WUInt8(addr, (byte)(1 - RUInt8(addr)));
            ...
            // Безопасное переключение невидимости для зомби
            chkBlindZombies.MouseUp += (s, e) => {
                var cheats = localPlayer?.DaytonPlayerController?.CheatManager;
                if (cheats != null) cheats.bInvisibleToZombies = !cheats.bInvisibleToZombies;
            };
            chkBlindZombies.KeyUp += (s, e) => {
                var cheats = localPlayer?.DaytonPlayerController?.CheatManager;
                if (cheats != null) cheats.bInvisibleToZombies = !cheats.bInvisibleToZombies;
            };

            // Безопасное управление спавнером
            chkSpawnerActive.MouseUp += (s, e) => {
                var spawner = gameMode?.DynamicPawnSpawner;
                if (spawner != null) spawner.Active = (byte)(1 - spawner.Active);
            };
            chkSpawnerActive.KeyUp += (s, e) => {
                var spawner = gameMode?.DynamicPawnSpawner;
                if (spawner != null) spawner.Active = (byte)(1 - spawner.Active);
            };
        ...
        {
            // Проверяем всю цепочку. Если world или уровни null, условие просто не выполнится.
            if (world?.PersistentLevel?.WorldSettings != null)
            {
                if (world.PersistentLevel.WorldSettings.TimeDilation > 0.001)
                {
                    world.PersistentLevel.WorldSettings.TimeDilation /= 2;
                }
            }
        }
        ...
        private void btnTimeDilationPlus_Click(object sender, EventArgs e)
        {
            if (world?.PersistentLevel?.WorldSettings != null)
            {
                if (world.PersistentLevel.WorldSettings.TimeDilation < 1000)
                {
                    world.PersistentLevel.WorldSettings.TimeDilation *= 2;
                }
            }
        ...
        {
            // 1. Проверяем, инициализированы ли объекты управления логами
            if (gamelogs == null || logNames == null)
            {
                Output("Error: Game logs not initialized.");
                return;
            }

            foreach (string logName in logNames)
            {
                // 2. Получаем адрес и приводим к IntPtr
                IntPtr addr = (IntPtr)gamelogs.Get(logName);

                // 3. Записываем только если адрес валиден
                if (addr != IntPtr.Zero)
                {
                    WUInt8(addr, 0);
                }
            }
            Output("All logs set to Minimum.");
        }
        ...
        private void btnGameLogLogLevelMax_Click(object sender, EventArgs e)
        {
            ...
            Output("All logs set to Maximum.");
        }
        ...
        {
            // 1. Проверяем, существует ли объект world
            if (world == null)
            {
                MessageBox.Show("Ошибка: Объект World не инициализирован.", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            // 2. Используем безопасный доступ к цепочке свойств
            var persistentLevel = world.PersistentLevel;

            // Проверяем уровень и настройки мира на null
            if (persistentLevel?.WorldSettings == null)
            {
                MessageBox.Show("Не удалось получить доступ к настройкам мира.", "Внимание", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            var worldSettings = persistentLevel.WorldSettings;

            string input = ShowNumericInputDialog(
                "Edit Time Dilation",
                "Enter new time dilation",
                worldSettings.TimeDilation.ToString(),
                isFloat: true
            );

            if (!string.IsNullOrWhiteSpace(input))
            {
                // Используем TryParse, чтобы программа не вылетала, если пользователь введет текст вместо числа
                if (float.TryParse(input, out float newDilation))
                {
                    worldSettings.TimeDilation = newDilation;
                }
                else
                {
        ...
        {
            // 1. Извлекаем объекты цепочкой с проверкой на null
            var mapState = localPlayer?.DaytonPlayerController?.MapUI?.MapState;
            var mapUI = localPlayer?.DaytonPlayerController?.MapUI;
            var capsule = currDaytonHumanCharacter?.CapsuleComponent;

            // 2. Проверяем, что все необходимые компоненты существуют
            if (mapState == null || mapUI == null)
            {
                Output("Error: Map system not initialized. Open the map in-game.");
                return;
            }

            if (capsule == null)
            {
                Output("Error: Character capsule not found. Ensure you are controlling a character.");
                return;
            }

            // 3. Проверяем наличие вейпоинта
            if (mapState.numWaypointPositions > 0)
            {
                // Берем позицию первого вейпоинта
                Vector2 wpPos = mapState.WaypointPositions[0];

                // Получаем границы карты
                Vector2 mapMin = mapUI.MapBoundsMin;
                Vector2 mapMax = mapUI.MapBoundsMax;

                // Расчет нормализованного размера карты (разница между Max и Min)
                Vector2 normMapMax = new Vector2(
                    Math.Abs(mapMax.X) + Math.Abs(mapMin.X),
                    Math.Abs(mapMax.Y) + Math.Abs(mapMin.Y)
                );

                // Преобразование координат вейпоинта (0.0 - 1.0) в мировые координаты
                wpPos = wpPos * normMapMax;
                wpPos = wpPos - (new Vector2(Math.Abs(mapMin.X), Math.Abs(mapMin.Y)));

                // Телепортация: меняем X и Y, а Z (высоту) задираем вверх на 10000 единиц, 
                // чтобы персонаж не заспавнился внутри земли, а плавно упал на нее.
                capsule.WarpPos = new Vector3(wpPos.X, wpPos.Y, capsule.WarpPos.Z + 10000);

                Output($"Warping to: {wpPos.X:F2}, {wpPos.Y:F2}");
            }
            else
            {
                Output("No MapUI waypoints found. Set a waypoint on the map first.");
            }
        }
